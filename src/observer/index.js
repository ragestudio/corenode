...
const callbackKey = Symbol('callback-key')
const targetsKey = Symbol('targets-key')
...
const validObservableOptionKeys = { async: 1 }

const processObserveOptions = options => {
...
const observe = function (observer, options) {
...
const propertiesBluePrint = { [oMetaKey]: { value: null }, observe: { value: observe }, unobserve: { value: unobserve } }

const prepareObject = (source, oMeta) => {
...
const prepareArray = (source, oMeta) => {
...
const prepareTypedArray = (source, oMeta) => {
    propertiesBluePrint[oMetaKey].value = oMeta;
    Object.defineProperties(source, propertiesBluePrint);
    return source;
}

const filterChanges = (options, changes) => {
    let result = changes;
    if (options) {
        if (options.path) {
...
const callObserverSafe = (listener, changes) => {
    try {
        listener(changes);
    } catch (e) {
...
const callObserversFromMT = function () {
...
const callObservers = (oMeta, changes) => {
...
const getObservedOf = (item, key, parent) => {
...
const proxiedSort = function (comparator) {
    const
        oMeta = this[oMetaKey],
        target = oMeta.target;
    let i, l, item;
...
const proxiedFill = function (filVal, start, end) {
    const oMeta = this[oMetaKey]
...
const proxiedCopyWithin = function (dest, start, end) {
    const oMeta = this[oMetaKey]
    const target = oMeta.target
    const tarLen = target.length
...
const proxiedSplice = function () {
    const oMeta = this[oMetaKey]
    const target = oMeta.target
    const splLen = arguments.length
...
const proxiedTypedArraySet = function (source, offset) {
    const oMeta = this[oMetaKey]
    const target = oMeta.target
