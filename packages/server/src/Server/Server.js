...
import { lodash, portfinder, createDebug } from '@nodecorejs/utils';
...
class Server {
    ...
    deleteRoutes() {
        ...
        this.app._router.stack.forEach((item, index) => {
            if (item.name === 'PROXY_START')
                startIndex = index;
            if (item.name === 'PROXY_END')
                endIndex = index;
        });
        debug(`routes before changed: ${this.app._router.stack
            .map((item) => item.name || 'undefined name')
            .join(', ')}`);
        if (startIndex !== null && endIndex !== null) {
            this.app._router.stack.splice(startIndex, endIndex - startIndex + 1);
        }
        debug(`routes after changed: ${this.app._router.stack
            .map((item) => item.name || 'undefined name')
            .join(', ')}`);
    }
    /**
            ...
            if (proxyConfig.target) {
                return createProxyMiddleware(context, {
                    ...
                    onProxyRes(proxyRes, req, res) {
                        const target = typeof proxyConfig.target === 'object'
