...
function initSocket({ url, onMessage, }) {
...
test('normal', async () => {
    ...
    const { body: compilerBody } = await got(`http://${hostname}:${port}/compiler`);
...
test('compress', async () => {
    ...
    const { body: compilerBody, headers } = await got(`http://${hostname}:${port}/compiler`, {
        headers: {
...
test('https', async () => {
    ...
    const { body: compilerBody, headers } = await got(`https://${hostname}:${port}/compiler`, {
...
describe('proxy', () => {
    ...
    it('proxy bypass', async () => {
        ...
        const { body: compilerBody } = await got(`http://${hostname}:${port}/compiler`);
        expect(compilerBody).toEqual('compiler');
        const { body: bypassBody } = await got(`http://${hostname}:${port}/api/compiler`);
