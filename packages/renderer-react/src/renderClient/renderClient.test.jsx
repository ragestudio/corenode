...
test('normal', async () => {
    ...
    const { container } = render(renderClient({
        history,
        plugin,
...
test('normal with mount', async () => {
    ...
    expect(container.outerHTML).toEqual('<div id="app"><div><h1>foo</h1></div></div>');
...
test('preloadComponent', async () => {
    ...
    const routes = [
        {
            ...
            component: dynamic({
                ...
                loader: async () => (props) => (<div className="layout">{props.children}</div>),
            }),
            routes: [
                {
                    path: '/foo',
    ...
    // /foo
    const { container: originContainer } = render(renderClient({
        history,
        plugin,
        ssrProps: {},
        rootElement: undefined,
...
test('preloadComponent routeChange with ssr', async () => {
    ...
    expect(container.innerHTML).toEqual('<div class="layout"><div>loading</div></div>');
    await waitFor(() => getByText(container, '/bar'));
    expect(container.innerHTML).toEqual('<div class="layout"><h1>/bar</h1></div>');
});
