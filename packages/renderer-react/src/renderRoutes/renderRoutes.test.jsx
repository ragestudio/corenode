    ...
    routes: [
        {
          ...
          <h2 data-testid="routes">
            {props.routes.map((r) => r.path).join(',')}
          </h2>
          {props.children}
        </>),
            routes: [
        ...
        },
        {
            path: '/users/:id',
            component: (props) => {
                return <h1 data-testid="test">{props.match.params.id}</h1>;
            },
        },
        ...
        },
        {
            path: '/props-route',
            foo: 'bar',
            component: (props) => (<h1 data-testid="test">{`${props.route.path} ${props.route.foo}`}</h1>),
        },
        {
            path: '/pass-props',
            component: (props) => {
            ...
            routes: [
                {
                    path: '/pass-props',
                    component: (props) => <h1 data-testid="test">{props.foo}</h1>,
                },
            ],
        },
        {
...
test('/layout-without-component', async () => {
    render(<MemoryRouter initialEntries={['/layout-without-component/foo']}>
      {routes}
    </MemoryRouter>);
    expect((await screen.findByTestId('test')).innerHTML).toEqual('Foo');
});
...
test('/bar-exact/foo', async () => {
    render(<MemoryRouter initialEntries={['/bar-exact/foo']}>{routes}</MemoryRouter>);
    expect((await screen.findByTestId('test')).innerHTML).toEqual('Fallback');
});
test('/bar-strict', async () => {
    render(<MemoryRouter initialEntries={['/bar-strict']}>{routes}</MemoryRouter>);
    expect((await screen.findByTestId('test')).innerHTML).toEqual('Fallback');
});
test('/BAR-sensitive', async () => {
    render(<MemoryRouter initialEntries={['/BAR-sensitive']}>{routes}</MemoryRouter>);
    expect((await screen.findByTestId('test')).innerHTML).toEqual('Fallback');
});
...
test('/props-route', async () => {
    render(<MemoryRouter initialEntries={['/props-route']}>{routes}</MemoryRouter>);
    expect((await screen.findByTestId('test')).innerHTML).toEqual('/props-route bar');
});
test('/pass-props', async () => {
    const { container } = render(<MemoryRouter initialEntries={['/pass-props']}>{routes}</MemoryRouter>);
    await waitFor(() => getByText(container, 'bar'));
    expect((await screen.findByTestId('test')).innerHTML).toEqual('bar');
});
test('/get-initial-props', async () => {
    const newRoutes = renderRoutes(routerConfig);
    const { container } = render(<MemoryRouter initialEntries={['/get-initial-props']}>
      {newRoutes}
    </MemoryRouter>);
    await waitFor(() => getByText(container, 'bar'));
    expect((await screen.findByTestId('test')).innerHTML).toEqual('bar');
...
test('/get-initial-props-without-unmount', async () => {
    const newRoutes = renderRoutes(routerConfig);
    const { container } = render(<MemoryRouter initialEntries={['/get-initial-props-without-unmount']}>
      {newRoutes}
    </MemoryRouter>);
    await waitFor(() => getByText(container, 'bar'));
...
test('/get-initial-props-embed', async () => {
    const newRoutes = renderRoutes(routerConfig);
    const { container } = render(<MemoryRouter initialEntries={['/get-initial-props-embed']}>
      {newRoutes}
    </MemoryRouter>);
    await waitFor(() => getByText(container, 'bar'));
...
test('/wrappers', async () => {
    const { container } = render(<MemoryRouter initialEntries={['/wrappers']}>{routes}</MemoryRouter>);
    expect(container.innerHTML).toEqual('<h1>wrapper 1 /wrappers</h1><h1>wrapper 2</h1><h1>foo</h1>');
});
test('/fallback-20140924', async () => {
    render(<MemoryRouter initialEntries={['/fallback-20140924']}>
      {routes}
    </MemoryRouter>);
    expect((await screen.findByTestId('test')).innerHTML).toEqual('Fallback');
});
