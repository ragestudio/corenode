...
readdirSync(fixtures).forEach((fixture) => {
    ...
    fn(fixture, async () => {
        ...
        try {
            config = Object.assign({}, config, require(join(cwd, 'config.ts')).default);
        }
        catch (e) { }
        // init bundler
        ...
        let indexCSS = '';
        try {
            indexCSS = readFileSync(join(cwd, config.outputPath, 'index.css'), 'utf-8');
        }
        catch (e) { }
        require(join(cwd, 'expect.ts')).default({
            indexJS: readFileSync(join(cwd, config.outputPath, 'index.js'), 'utf-8'),
            indexCSS,
            files: readdirSync(join(cwd, config.outputPath)).filter((f) => f.charAt(0) !== '.'),
            cwd,
