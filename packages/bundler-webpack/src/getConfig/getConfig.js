...
import { getBabelDepsOpts, getBabelOpts, getBabelPresetOpts, getTargetsAndBrowsersList, } from '@nodecorejs/bundler-utils';
import { lodash } from '@nodecorejs/libs';
import css, { createCSSRule } from './css';
import terserOptions from './terserOptions';
import { TYPE_ALL_EXCLUDE, isMatch, excludeToPkgs, es5ImcompatibleVersionsToPkg, } from './nodeModulesTransform';
import resolveDefine from './resolveDefine';
export default async function getConfig(opts) {
    const { cwd, config, type, env, entry, hot, port, bundleImplementor = defaultWebpack, modifyBabelOpts, modifyBabelPresetOpts, miniCSSExtractPluginPath, miniCSSExtractPluginLoaderPath, } = opts;
    ...
    if (process.env.UMI_DIR) {
        ...
        webpackConfig.module
            .rule('js')
            .test(/\.(js|mjs|jsx|ts|tsx)$/)
            .include.add(join(process.env.UMI_DIR, 'dist', 'index.esm.js')).end()
            .use('babel-loader')
            .loader(require.resolve('babel-loader'))
            .options(babelOpts);
    }
    ...
    rule.use('babel-loader').loader(require.resolve('babel-loader')).options(getBabelDepsOpts({
        cwd,
        env,
        config,
    }));
    ...
    if (process.env.WEBPACK_PROFILE) {
        ...
        webpackConfig.stats((statsInclude.includes(process.env.WEBPACK_PROFILE)
