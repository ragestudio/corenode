  ...
  function applyLoaders(rule, isCSSModules) {
    if (config.styleLoader) {
      ...
      if (type === _types().BundlerConfigType.csr && !config.styleLoader) {
        rule.use('extract-css-loader').loader(miniCSSExtractPluginLoaderPath || require('../../mini-css-extract-plugin/dist/index').default.loader).options({
          publicPath: './',
          hmr: isDev
        });
      }
    ...
    rule.use('css-loader').loader(require.resolve('css-loader')).options((0, _utils().deepmerge)(_objectSpread(_objectSpread({
      importLoaders: 1
...
function _default({
  ...
  if (!config.styleLoader) {
    ...
    if (type === _types().BundlerConfigType.csr) {
      webpackConfig.plugin('extract-css').use(miniCSSExtractPluginPath || require('../../mini-css-extract-plugin/dist/index').default, [{
