...
var _default = api => {
  ...
  api.chainWebpack((webpackConfig, opts) => {
    ...
    if (type == _types().BundlerConfigType.csr && !process.env.ANALYZE_SSR || type === _types().BundlerConfigType.ssr && !process.env.ANALYZE) {
      ...
      webpackConfig.plugin('bundle-analyzer').use(require('umi-webpack-bundle-analyzer').BundleAnalyzerPlugin, [((_api$config = api.config) === null || _api$config === void 0 ? void 0 : _api$config.analyze) || {}]);
    }
