...
export function generateExports({ item, coreExportsHook, }) {
    ...
    assert(!reserveLibrarys.includes(item.source), `${item.source} is reserve library, Please don't use it.`);
    if (item.exportAll) {
    ...
    const specifiersStrArr = item.specifiers.map((specifier) => {
        if (typeof specifier === 'string') {
            ...
            assert(!coreExportsHook[specifier], `${specifier} is Defined, you can use 'exported' to set alias.`);
