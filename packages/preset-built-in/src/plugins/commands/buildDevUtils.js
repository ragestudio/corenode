...
export async function getBundleAndConfigs({ api, port, }) {
    ...
    const bundleConfigs = await api.applyPlugins({
        ...
        initialValue: [await getConfig({ type: BundlerConfigType.csr })].filter(Boolean),
        args: {
            ...bundlerArgs,
            getConfig,
        },
...
export function printFileSizes(stats, dir) {
    ...
    ui.div(makeRow(chalk.cyan.bold(`File`), chalk.cyan.bold(`Size`), chalk.cyan.bold(`Gzipped`)) +
        ...
        orderedAssets
            ...
            : chalk.blue(join(dir, asset.name)), filesize(asset.size), getGzippedSize(asset)))
            .join(`\n`));
    ...
    if (orderedAssets?.some((asset) => asset.suggested)) {
        ...
        console.log(chalk.yellow('Consider reducing it with code splitting: https://nodecorejs.org/docs/load-on-demand'));
