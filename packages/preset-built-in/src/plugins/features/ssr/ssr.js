...
import { CHUNK_NAME, OUTPUT_SERVER_FILENAME, TMP_PLUGIN_DIR, CLIENT_EXPORTS, } from './constants';
/**
 * onBuildComplete for test case
 * replace default html template using client webpack bundle complete
 * @param api
 */
export const onBuildComplete = (api, _isTest = false) => async ({ err, stats, }) => {
    if (!err && stats?.stats) {
        const HTML_REG = /<html.*?<\/html>/m;
        const [clientStats] = stats.stats;
        const html = getHtmlGenerator({ api });
        const [defaultHTML] = JSON.stringify(await html.getContent({
            route: { path: api.config.publicPath },
            chunks: clientStats.compilation.chunks,
        })).match(HTML_REG) || [];
        const serverPath = path.join(api.paths.absOutputPath, OUTPUT_SERVER_FILENAME);
...
export default (api) => {
        ...
        config: {
            schema: (joi) => {
                return joi
                    ...
                    .error(new Error('The `removeWindowInitialProps` cannot be enabled when `forceInitial` has been enabled at the same time.'));
            },
        },
        // 配置开启
        enableBy: () => 
    ...
    api.onStart(() => {
        assert(api.config.history?.type !== 'hash', 'the `type` of `history` must be `browser` when using SSR');
        ...
        if (!process.env.WATCH_IGNORED) {
            ...
            const absOutputPath = winPath(path.join(api.cwd, outputPath, '/'));
    ...
    api.onGenerateFiles(async () => {
        ...
        api.writeTmpFile({
            ...
            content: Mustache.render(serverContent, {
                ...
                RuntimePath: winPath(path.dirname(require.resolve('@nodecorejs/runtime/package.json'))),
                Renderer: winPath(require.resolve('./templates/renderServer/renderServer')),
                RuntimePolyfill: winPath(require.resolve('regenerator-runtime/runtime')),
    ...
    api.modifyHTMLChunks(async (memo, opts) => {
        ...
        if (api.config.dynamicImport &&
            ...
            const chunkArr = [];
            const routes = await api.getRoutes();
            const matchedRoutes = matchRoutes(routes, route.path);
            const chunks = _.uniq(matchedRoutes.map((matchedRoute) => matchedRoute.route.component
    ...
    api.modifyConfig((config) => {
        ...
        config.devServer.writeToDisk = (filePath) => {
            const manifestFile = api.config?.manifest?.fileName || 'asset-manifest.json';
