...
test('api.writeTmpFile error in register stage', async () => {
    ...
    await expect(service.init()).rejects.toThrow(/api.writeTmpFile\(\) should not execute in register stage./);
});
...
test('html', async () => {
    ...
    expect(removeSpace($('head script').eq(3).html())).toContain(`console.log(3)`);
    expect($('body script[src="//g.alicdn.com/react.js"]')).toBeTruthy();
    expect(removeSpace($('body script').eq(1).html())).toContain(`console.log(1);`);
    expect(removeSpace($('body script').eq(2).html())).toContain(`console.log(2);`);
    expect($('body script[crossorigin="true"]').attr('src')).toEqual('/custom.js');
});
test('ssr', async () => {
    ...
    const expectRootContainer = '<div><ul><li>hello</li><li>world</li></ul></div>';
...
test('ssr beforeRenderServer', async () => {
    ...
    const expectRootContainer = '<div><h1>/</h1><ul><li>hello</li><li>world</li></ul></div>';
    expect(rootContainer).toEqual(expectRootContainer);
    const $ = cheerio.load(html);
    expect($('#root').html()).toEqual(expectRootContainer);
    rimraf.sync(join(cwd, '.umi-test'));
...
test('ssr using stream', (done) => {
    ...
    service
        ...
        render({
            ...
            const expectBytes = Buffer.from('<div><ul><li>hello</li><li>world</li></ul></div>');
...
test('ssr dynamicImport', async () => {
    ...
    expect(homeResult.html).toMatch('<link rel="stylesheet" href="/p__index.chunk.css" />');
    // render /bar
    ...
    expect(BarResult.html).toMatch('<link rel="stylesheet" href="/p__Bar.chunk.css" />');
    rimraf.sync(join(cwd, '.umi-test'));
});
