...
test('normal', () => {
    ...
    const ret = parseRequireDeps(join(fixture, '.umirc.ts')).map((p) => p.replace(winPath(fixture), '.'));
    expect(ret).toEqual(['./.umirc.ts', './config/foo.ts', './src/a.js']);
});
test('directory index', () => {
    const fixture = join(fixtures, 'directory-index');
    const ret = parseRequireDeps(join(fixture, 'config/config.ts')).map((p) => p.replace(winPath(fixture), '.'));
    expect(ret).toEqual([
        './config/config.ts',
        './utils/index.tsx',
        './src/foo.ts',
