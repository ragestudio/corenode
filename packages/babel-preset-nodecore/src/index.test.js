import { deepmerge, winPath } from '@nodecorejs/libs';
...
test('typescript with metadata', () => {
    const code = transformWithPreset(`@Decorate
    class MyClass {
      constructor(
...
test('typescript with nest-injection', () => {
    const code = transformWithPreset(`import { AppService } from './app.service';
...
test('typescript key remapping types', () => {
    const code = transformWithPreset(`type Options = {
...
test('transform runtime', () => {
    ...
    expect(winPath(join(code))).toContain(`node_modules/@babel/runtime/helpers/esm/classCallCheck"));`);
});
...
test('babel-plugin-react-require', () => {
    ...
    expect(code).toContain('var _react = _interopRequireDefault(require("react"));');
});
