...
const simplyPluginIds = ({ cwd, plugins }) => Object.keys(plugins).map((id) => {
...
test('applyPlugin with add failed with non-array initialValue', async () => {
    ...
    await service.init();
    await expect(service.applyPlugins({
        key: 'test',
        type: ApplyPluginsType.add,
        initialValue: '',
...
test('api.registerMethod fail if exist', async () => {
    ...
    await expect(service.init()).rejects.toThrow(/api\.registerMethod\(\) failed, method foo is already exist/);
});
